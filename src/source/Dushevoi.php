...
class Dushevoi{
    ...
    public function getCategories($cats=[],callable $_callback=null){
        $cat = new Category;
        $li = ['external_id' => "dushevie",'title' => "Душевые кабины", 'url' => "https://www.dushevoi.ru/products/dushevye-kabiny/",'brands'=>[] ];
        $cat->fromArray($li);
        $this->categories[$li["external_id"]] = $li;
        $this->internal_categories[$li["external_id"]] = (!is_null($_callback))?$_callback($cat):null;
        return;
    ...
    protected function productsFromPage($prodUrl,$cat_id){
        ...
        $vendor = $pp->find("#attrs [itemprop=additionalProperty] [itemprop=name]")->text();
        // $alowedBrands = (isset($this->categories[$cat_id]) && isset($this->categories[$cat_id]["brands"]) && count($this->categories[$cat_id]["brands"]))?$this->categories[$cat_id]["brands"]:[$brand];
        // if(!in_array($brand,$alowedBrands)){
        //     Log::console("Wrond brand: ".$brand);
        //     return null;
        // }
        // $pics = $pdoc[".preview ul.prew li a img"];
        $pics = $pdoc[".well.thumbs .thumbnail .trans"];
        foreach ($pics as $pic) $pictures[]="http://".pq($pic)->attr("src");
